...
def loadModel(modelName, input_shape, num_classes):
    ...
    if modelName=="uocSeq1":#Model with two convolutional layers
        model = Sequential()
        model.add(Conv2D(32, kernel_size=(3, 3), strides=(1, 1), padding=Padding,activation=Activation_functions, input_shape=input_shape))
        model.add(Conv2D(32, (3, 3), strides=(1, 1), padding=Padding, activation=Activation_functions))
        model.add(Pooling)
